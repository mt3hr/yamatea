...
void initializeCommandExecutor(CommandExecutor *commandExecutor, RobotAPI *robotAPI)
{
  ...
  Predicate *facingAnglePredicate = new ORPredicate(new FinishedCommandPredicate(facingAngle), new TimerPredicate(waitDurationUsec));
...
#if defined(TrueCourceSanekataModeSimulator)
void initializeCommandExecutor(CommandExecutor *commandExecutor, RobotAPI *robotAPI)
{
  // カラーセンサの乗ったアームの角度を調節する
  ResetArmAngle *resetArmAngle = new ResetArmAngle();
...
// ↓ここから実方↓
...
#endif
  {
    ...
    waterMelonDistance = 254 * coefficientDistance;         // Simulator
