...
// テールモータで角度をつける
  ...
  */
  ...
  pwm = 7 * coefficientPWM;
  distance = 9;
  HedgehogUsePID *headgehog1 = new HedgehogUsePID(distance, pwm, straightKp, straightKi, straightKd, straightDt);
  commandExecutor->addCommand(headgehog1, new FinishedCommandPredicate(headgehog1), GET_VARIABLE_NAME(headgehog1));
  commandExecutor->addCommand(stopper, new NumberOfTimesPredicate(1), GET_VARIABLE_NAME(stopper));
  ...
  walkerD->setTargetDifferenceWheelCount(0);
  WheelDistancePredicate *walkerDPredicate = new WheelDistancePredicate(distance, robotAPI);
  commandExecutor->addCommand(walkerD, walkerDPredicate, GET_VARIABLE_NAME(walkerD));
  commandExecutor->addCommand(stopper, new NumberOfTimesPredicate(1), GET_VARIABLE_NAME(stopper));
  ...
  distance = 2;
  pwm = 7;
  PIDStraightWalker *walkerZ = new PIDStraightWalker(pwm, straightKp, straightKi, straightKd, straightDt);
  walkerZ->setTargetDifferenceWheelCount(0);
  WheelDistancePredicate *walkerZPredicate = new WheelDistancePredicate(distance, robotAPI);
  commandExecutor->addCommand(walkerZ, walkerZPredicate, GET_VARIABLE_NAME(walkerZ));
  commandExecutor->addCommand(stopper, new NumberOfTimesPredicate(1), GET_VARIABLE_NAME(stopper));
  ...
  */
  ...
  pwm = 7 * coefficientPWMForFacingAngle;
  angle = -90;
  PIDFacingAngleAbs *facingAngleY = new PIDFacingAngleAbs(facingAngleMode, slalomAngleOffset + angle);
  ...
  pwm = 5 * coefficientPWMForFacingAngle;
  angle = -150;
  PIDFacingAngleAbs *facingAngle8 = new PIDFacingAngleAbs(facingAngleMode, slalomAngleOffset + angle);
  ...
  pwm = 7 * coefficientPWMForFacingAngle;
  angle = -30;
  PIDFacingAngleAbs *facingAngle9 = new PIDFacingAngleAbs(facingAngleMode, slalomAngleOffset + angle);
  Predicate *facingAngle9Predicate = new ORPredicate(new FinishedCommandPredicate(facingAngle9), new TimerPredicate(waitFaUsec));
  commandExecutor->addCommand(facingAngle9, facingAngle9Preicate, GET_VARIABLE_NAME(facingAngle9));
  commandExecutor->addCommand(stopper, new NumberOfTimesPredicate(1), GET_VARIABLE_NAME(stopper));
  ...
  */
  ...
  pwm = 7 * coefficientPWMForFacingAngle;
  angle = -120;
  PIDFacingAngleAbs *facingAngle10 = new PIDFacingAngleAbs(facingAngleMode, slalomAngleOffset, faKp, faKi, faKd, faDt);
...
void initializeCommandExecutor(CommandExecutor *commandExecutor, RobotAPI *robotAPI)
{
  ...
  commandExecutor->addCommand(pidFacingAngleAbs, new FinishedCommandPredicate(pidFacingAngleAbs), GET_VARIABLE_NAME(pidFacingAngleAbs));
}
#endif
